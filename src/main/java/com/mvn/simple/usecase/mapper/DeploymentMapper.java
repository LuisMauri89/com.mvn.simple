...
public class DeploymentMapper {
	...
	public CustomDeployment toCustomDeployment(
			final DeploymentInfo info, 
			final LocationMapper locationMapper, 
			final CollisionsMapper collisionsMapper) throws Exception {
		List<String> topRightCoordinate = Arrays.asList(info.getTopRightCoordinate().split(" "))
				.stream()
				.map(s -> s.trim())
		...
		CustomDeployment deployment = CustomDeployment.builder()
				.name(info.getName())
	...
	private List<MowerHandler> mapMowersHandlers(
			final DeploymentInfo info, 
			final LocationMapper locationMapper) throws Exception{
		...
		for(int i = 0; i < info.getMowersInfo().size(); i += 2) {
			List<String> positionAndDirection = Arrays.asList(info.getMowersInfo().get(i).split(" "))
					.stream()
					.map(s -> s.trim())
					.collect(Collectors.toList());
			...
			try {
				mower.setCurrentPosition(new Point(
								Integer.parseInt(positionAndDirection.get(0)), 
			...
			CustomMowerHandler handler = CustomMowerHandler.builder()
					.mower(mower)
